...
import com.seija.math.{Quat, Vector3}
...
object FFISeijaTransform {
  ...
  def transformSetParent(worldPtr:Ptr[Byte],entity:Long,parent:Long,isNull:Boolean) = {
      transformSetParentPtr(com.seija.core.App.worldPtr,entity,parent,isNull)
  }

  def transformAddChildIndex(curEntity:Long,childEntity:Long,index:Int):Unit = {
      transformAddChildIndexPtr(com.seija.core.App.worldPtr,curEntity,childEntity,index);
  }

  def transformDespawn(curEntity:Long):Unit = {
      transformDespawnPtr(com.seija.core.App.worldPtr,curEntity);
  }

  def setActive(entity:Entity,active:Boolean):Unit = transform_set_activePtr(com.seija.core.App.worldPtr,entity.id,active)

  def isActiveGlobal(entity:Entity):Boolean = transform_is_active_globalPtr(com.seija.core.App.worldPtr,entity.id)

  def addStateInfo(entity:Entity,active:Boolean):Unit = {
    transform_add_state_infoPtr(com.seija.core.App.worldPtr,entity.id,active)
  }

  def calcRelative(child:Entity,parent:Option[Entity]):TransformMatrix = {
    val outStackPtr = stackalloc[STransform]()
    parent match {
      case Some(value) => transform_relative_toPtr(com.seija.core.App.worldPtr, outStackPtr, child.id, value.id, false)
      case None => transform_relative_toPtr(com.seija.core.App.worldPtr, outStackPtr, child.id, 0, true)
    }
