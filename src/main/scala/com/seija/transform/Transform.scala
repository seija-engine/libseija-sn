...
import com.seija.math.{Vector3, Quat}
...
class TransformBuilder extends RawComponentBuilder {
    ...
    def build(entity:Entity):Unit = {
        FFISeijaTransform.transformAdd(com.seija.core.App.worldPtr,entity,this)
        if(parent.isDefined) {
            FFISeijaTransform.transformSetParent(com.seija.core.App.worldPtr,entity.id,parent.get.id,false)
        }
    }
}   
...
object Transform {
  given TransformComponent:RawComponent[Transform] with {
    ...
    def getRaw(entity:Entity,isMut:Boolean):RawTransform = FFISeijaTransform.transformGet(com.seija.core.App.worldPtr,entity.id)
  }
