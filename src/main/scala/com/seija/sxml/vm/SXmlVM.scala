...
import com.seija.sxml.vm.stdlib.IOModule
...
class SXmlVM {
    ...
    def callCodeSource(codeName:String,source:Source):Try[VMValue] = Try {
        val parser = com.seija.sxml.parser.Parser.fromSource(codeName,source)
        val astModule = parser.parseModule().get
        val trans = com.seija.sxml.compiler.Translator()
        val transModule = trans.translateModule(astModule).get
        val compiler = com.seija.sxml.compiler.Compiler(this.env)
