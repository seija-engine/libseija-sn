package com.seija.slog.throwable
import com.seija.slog.{LoggableMessage}
import com.seija.slog.output.{CompositeOutput, EmptyOutput, LogOutput, TextOutput}
    ...
    trace.cause match {
      ...
      case None =>
        ...
        if (message == EmptyOutput) {
          ...
          new CompositeOutput(List(message, new TextOutput(slog.lineSeparator), output))
        }
    }
  }
